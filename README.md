...
脚本会自动关闭正在运行的 Antigravity 进程，并执行汉化注入。等待提示“汉化成功完成”后，重新打开 Antigravity 即可看到中文界面。

### 方法二：命令行静默执行

如果你已经打开了终端或 PowerShell，可以直接执行以下命令，在后台快速完成汉化：
```bash
...
1. 在当前目录打开终端。
2. 运行命令：`node localize.js`
3. 打开浏览器访问：`http://localhost:3388`
4. 在可视化的控制面板中，你可以清晰地看到安装状态，并可以点击“一键开始汉化”或“恢复英文原版”。
